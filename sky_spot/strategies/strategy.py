...
class Strategy:
    ...
    def reset(self, env: "env.Env", task: "task_lib.Task"):
        ...
        self.deadline: float = args.deadline_hours * 3600
    ...
    def _apply_strong_guarantee(
        self, proposed_decision: ClusterType, last_cluster_type: ClusterType
    ) -> ClusterType:
        """The non-negotiable final check."""
        # Check if this strategy opts out of safety net
        if getattr(self, "IGNORE_SAFETY_NET", False):
        ...
        if total_task_remaining >= remaining_time:
            # The preemption of a working spot won't affect finishing the task in time. Because even if the spot is preempted,
            # we still have enough time for launching a new OD and pay for that task_remaining_time.
            # Even if the strategy wants to launch a OD here, we override it to Spot. Since it won't become worse.
            if (
    ...
    def _before_decision_hook(
        self, last_cluster_type: ClusterType, request_type: ClusterType
    ):
    ...
    def step(self) -> ClusterType:
        ...
        try:
            ...
            print("remaining task", self.task_duration - sum(self.task_done_time))
        ...
        if last_cluster_type == ClusterType.NONE:
            ...
            task_done_time = max(available_time - self.remaining_restart_overhead, 0)
            self.remaining_restart_overhead -= available_time - task_done_time
        ...
        request_type = self._apply_strong_guarantee(
            heuristic_decision, last_cluster_type
        )
        ...
        # If the SPOT is not available in the current region, and the critical window is reached, it must be switched to ON_DEMAND to ensure completion on time.
        # Otherwise, it can be set to NONE.
        if request_type == ClusterType.SPOT and not env.spot_available():
            # Reuse the strong guarantee check: whether the remaining discrete time required to complete (including one restart overhead) is already not less than the remaining time
            remaining_time = (
            ...
            in_critical_window = (
                total_task_remaining >= remaining_time and remaining_task_time > 1e-3
            )

            if in_critical_window:
                logger.warning(
                    f"Tick {env.tick}: SAFETY OVERRIDE - SPOT unavailable in critical window. Changing to ON_DEMAND."
                )
                request_type = ClusterType.ON_DEMAND
            else:
                logger.warning(
                    f"Tick {env.tick}: SAFETY OVERRIDE - SPOT requested but not available in current region. Changing to NONE."
                )
        ...
        if current_cluster_type != request_type and request_type != ClusterType.NONE:
            self.remaining_restart_overhead = self.restart_overheads[0]
        return request_type
    ...
    def from_args(cls, parser: "configargparse.ArgumentParser") -> "Strategy":
        ...
        parser.add_argument(
            "--strategy", type=str, default="strawman", choices=cls.SUBCLASSES.keys()
        )
    ...
    def info(self):
        ...
        strategy_info = {
            "Strategy/RemainingRestartOverhead(seconds)": self.remaining_restart_overhead,
            "Strategy/CurrentRestartOverheadIndex": current_restart_idx,
            "Strategy/CurrentInterTaskOverheadIndex": current_overhead_idx,
            "Strategy/RemainingInterTaskOverhead(seconds)": self.remaining_inter_task_overhead,
        }
