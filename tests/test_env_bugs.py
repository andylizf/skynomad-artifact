"""Regression tests pinning MultiTraceEnv's billing and termination semantics."""
...
class MockTask(task_lib.Task):
    ...
    def __init__(self, duration_seconds: float):
        ...
        # Ensure migration model has a default checkpoint size
...
class TestInstanceSwitching(unittest.TestCase):
        ...
        class TestStrategy(MultiRegionStrategy):
            ...
            def _step_multi(self) -> Generator[Action, Optional[LaunchResult], None]:
                ...
                if self._tick_count == 0:
                    ...
                    yield Terminate(region=0, cluster_type=ClusterType.ON_DEMAND)  # Explicitly terminate ON_DEMAND
    ...
    def test_launch_without_terminate_replaces(self):
        """Test that launching a different type requires explicit termination to maintain single-instance invariant."""
        env = MultiTraceEnv(self.trace_files, env_start_hours=0)
        
        class ReplacementStrategy(MultiRegionStrategy):
            """Strategy that explicitly replaces instance types."""
            NAME = 'test_replacement_strategy'
            ...
            def _step_multi(self) -> Generator[Action, Optional[LaunchResult], None]:
                ...
                if self._tick_count == 0:
                    ...
                    # Launch SPOT then terminate ON_DEMAND (explicit replacement)
                    result = yield TryLaunch(region=0, cluster_type=ClusterType.SPOT)
                    assert result is not None and result.success
        ...
        strategy = ReplacementStrategy(MockArgs())
        task = MockTask(duration_seconds=1800)
        ...
        env.execute_multi_strategy(strategy)
        active = env.get_active_instances()
        self.assertEqual(active[0], ClusterType.ON_DEMAND)
        env.tick += 1
        
        # Second tick: launch SPOT and explicitly terminate ON_DEMAND
        env.observe()
        env.execute_multi_strategy(strategy)
        active = env.get_active_instances()
        ...
        # Verify billing includes both types
        env.observe()
        self.assertEqual(len(env.cost_history), 2)
        self.assertEqual(env.cost_history[0], {0: ClusterType.ON_DEMAND})
