...
class TestTaskCompletion(unittest.TestCase):
    ...
    def test_multi_region_info_snapshot_no_oob(self):
        """Ensure info-stage SpotAvailability reads at tick-1 and does not OOB at boundary."""
        # Create a very short trace so that a single short task completes near boundary
        ...
        trace_data = {
            ...
            # 3 ticks total: indices 0,1,2
            "data": [0, 0, 0]
        }
        short_trace_file = os.path.join(self.temp_dir, "short_region_0_v100_1.json")
        with open(short_trace_file, 'w') as f:
            json.dump(trace_data, f)

        # Create environment and a task that will finish within the trace
        from sky_spot.env import MultiTraceEnv
        env = MultiTraceEnv([short_trace_file], env_start_hours=0)

        # Task duration = 2 ticks
        task = SimpleTask(duration_seconds=2 * gap)

        # Build a minimal multi-region strategy that launches once and does nothing further
        import argparse
        from sky_spot.strategies.strategy import MultiRegionStrategy
        from sky_spot.multi_region_types import TryLaunch
        from sky_spot.utils import ClusterType
        ...
        class LaunchOnceStrategy(MultiRegionStrategy):
            NAME = 'launch_once_test'
        ...
        strategy = LaunchOnceStrategy(args)

        # Run via simulate to execute the full info packaging path
        stats = simulate.simulate(
            envs=[env],
            strategy=strategy,
            task=task,
            trace_file=os.path.basename(short_trace_file),
            deadline_hours=2.0,
        ...
        # If we reach here without exceptions, no OOB occurred at info stage
        self.assertTrue(strategy.task_done)
