...
class MockTask(task_lib.Task):
    ...
    def __init__(self, duration_seconds: float):
        ...
        # Default checkpoint size used for migration overhead when needed
...
class AlwaysSpotStrategy(Strategy):
    ...
    NAME = 'always_spot_migration_test'
...
class TestRestartOverhead(unittest.TestCase):
    ...
    def setUp(self):
        ...
        class Args:
            ...
            strategy = 'always_spot_migration_test'
        ...
        try:
            ...
            for tick in range(10):
                if not self.task.is_done:
                    ...
                    # Update strategy progress (this now handles preemptions)
                    env.update_strategy_progress(strategy)
                    
                    # Check if region 0 was preempted and we need to migrate.
                    # With preemption evaluated on the previous tick (tick-1 semantics),
                    # region 0 becomes unavailable at tick 3 and is recognized at tick 4.
                    if tick == 4 and 0 not in env.get_active_instances():
