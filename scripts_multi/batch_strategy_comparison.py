...
def run_strategy_test(strategy_name, trace_ids, output_dir, timeout=300, strategy_files=None,
                     ...
                     strategy_extra_args=None,
                     main_extra_args=None,
    ...
    for trace_id in trace_ids:
        ...
        # Append optional extra args for this strategy
        if strategy_extra_args and strategy_name in strategy_extra_args:
            extra_str = strategy_extra_args[strategy_name]
            if isinstance(extra_str, str) and extra_str.strip():
                cmd.extend(shlex.split(extra_str))

        # Append optional extra args for main.py (global flags, e.g., --checkpoint-size-gb)
        if main_extra_args:
            if isinstance(main_extra_args, str):
                cmd.extend(shlex.split(main_extra_args))
            elif isinstance(main_extra_args, (list, tuple)):
                for s in main_extra_args:
                    if isinstance(s, str) and s.strip():
                        cmd.extend(shlex.split(s))
        ...
        try:
            if stream_logs:
                ...
                # Capture for compact summary output
                result = subprocess.run(cmd, capture_output=True, text=True, timeout=timeout)
            
            if result.returncode == 0:
                print(f"✅ {strategy_name} trace {trace_id} completed")
                
                # Find the actual output file (simulate.py uses hash-based naming)
                test_output_path = Path(test_output_dir)
                json_files = list(test_output_path.glob(f"{strategy_name}-multi_trace-*.json"))
                
                if not json_files:
                    # Fallback to any json file if strategy name doesn't match exactly
                    json_files = list(test_output_path.glob("*.json"))
                ...
                if json_files:
                    ...
                    results.append({
                        ...
                        'result_file': str(actual_file)
                    })
                else:
                    print(f"❌ Could not find output file for {strategy_name} trace {trace_id}")
                    results.append({
                        'trace_id': trace_id,
                        'strategy': strategy_name,
                        'cost': 'N/A',
                ...
                # If logs were captured, print stderr; otherwise, note that logs were streamed
                if stream_logs:
                    print(f"❌ {strategy_name} trace {trace_id} failed (see console logs)")
                    err_snippet = "(stderr not captured; see console)"
                else:
...
def generate_visualizations(results, output_dir):
    ...
    for result in results:
        if result['status'] == 'success' and 'result_file' in result:
            ...
            cmd = [
                sys.executable, "scripts_multi/visualize_timeline_segments.py",
                result['result_file'],
                "-o", viz_file
            ]
...
def main():
    ...
    parser.add_argument('--stream-logs', action='store_true',
                       help='Stream child process logs live instead of capturing output')
    ...
    parser.add_argument('--strategy-extra-args', nargs='*', metavar='STRATEGY=ARGS',
                       help='Append extra CLI args for a strategy (format: name="--flag1 v1 --flag2")')
    parser.add_argument('--main-extra-args', nargs='*', metavar='ARGS',
                       help='Append extra CLI args to main.py (e.g., "--checkpoint-size-gb 200")')
    parser.add_argument('--task-duration-hours', type=float, default=DEFAULT_TASK_DURATION_HOURS,
                       help=f'Task duration hours (default: {DEFAULT_TASK_DURATION_HOURS})')
    parser.add_argument('--deadline-hours', type=float, default=DEFAULT_DEADLINE_HOURS,
                       help=f'Deadline hours (default: {DEFAULT_DEADLINE_HOURS})')
    parser.add_argument('--restart-overhead-hours', type=float, default=DEFAULT_RESTART_OVERHEAD_HOURS,
                       help=f'Restart overhead hours (default: {DEFAULT_RESTART_OVERHEAD_HOURS})')
    ...
    # Parse strategy extra args mapping
    if args.strategy_extra_args:
        strategy_extra_args = {}
        for mapping in args.strategy_extra_args:
            if '=' in mapping:
                strategy_name, arg_str = mapping.split('=', 1)
                strategy_extra_args[strategy_name] = arg_str
            else:
                print(f"Warning: Invalid strategy extra-args mapping format: {mapping}")
        args.strategy_extra_args = strategy_extra_args
    else:
        args.strategy_extra_args = None
    ...
    for strategy in strategies:
        ...
        strategy_results = run_strategy_test(
            ...
            output_base_dir,
            args.timeout,
            getattr(args, 'strategy_files', None),
            task_duration_hours=args.task_duration_hours,
            deadline_hours=args.deadline_hours,
            restart_overhead_hours=args.restart_overhead_hours,
            strategy_extra_args=getattr(args, 'strategy_extra_args', None),
            main_extra_args=getattr(args, 'main_extra_args', None),
    ...
    # Check if any strategy completely failed
    has_failures = False
    for strategy in strategies:
        strategy_results = [r for r in final_results if r['strategy'] == strategy]
        successful = [r for r in strategy_results if r['status'] == 'success']
        if len(successful) == 0:
            print(f"\n⚠️ WARNING: Strategy {strategy} had no successful runs!")
            has_failures = True
    
    # Return non-zero exit code if any strategy completely failed
    if has_failures:
        sys.exit(1)
