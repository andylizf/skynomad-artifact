...
def analyze_region_durations(data_dir: Path, use_full_trace: bool = True,
    ...
    region_dirs = sorted([d for d in data_dir.iterdir() if d.is_dir() and d.name.endswith("v100_1")])
...
def plot_aggregated_distribution(region_durations: Dict[str, List[float]],
    ...
    ax.set_ylabel('Frequency', fontsize=15, labelpad=5)
...
def main():
    ...
    # Configuration
    data_dir = Path("data/converted_multi_region_aligned")
    ...
    # Save raw data
    suffix = "_v100_quantized" if quantize else "_v100"
